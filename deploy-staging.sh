...
docker build -t gcr.io/${PROJECT_NAME_STG}/${BACKEND_DOCKER_IMAGE_NAME}:$TRAVIS_COMMIT $BACKEND_BUILD_DIR
...
docker push gcr.io/${PROJECT_NAME_STG}/${BACKEND_DOCKER_IMAGE_NAME}
...
kubectl set image deployment/${BACKEND_KUBE_DEPLOYMENT_NAME} ${BACKEND_KUBE_DEPLOYMENT_CONTAINER_NAME}=gcr.io/${PROJECT_NAME_STG}/${BACKEND_DOCKER_IMAGE_NAME}:$TRAVIS_COMMIT
